...
def simple_test_net():
    ...
    pp.create_line(net, b6, b8, 2, "184-AL1/30-ST1A 20.0")
    ...
    pp.create_sgen(net, b7, p_mw=13, q_mvar=2, name='sgen4')
    pp.create_sgen(net, b8, p_mw=0.09, q_mvar=0.01, name='sgen5')
    pp.create_sgen(net, b8, p_mw=0.15, q_mvar=0.02, name='sgen6')
    pp.create_sgen(net, b9, p_mw=0.1, q_mvar=0.02, name='sgen7')
...
def create_data_source(n_timesteps, net):
    ...
    for i in range(6):
        ...
        v = rng.normal(loc=0.9, scale=0.1, size=100)
        cut = len(v) // 2
    ...
    for i in range(6):
        ...
        v = rng.normal(loc=0.8, scale=0.1, size=100)
        cut = len(v) // 2
    ...
    # Creazione dinamica del dizionario per il DataFrame
    profiles_dict = {}
    for i in range(6):
    ...
    # Debug: stampa i primi valori del profilo
...
def create_controllers(net, ds):
    # Gestione automatizzata dei carichi (load1-6)
    for i in range(1, 7):
        load_name = f"load{i}"
    ...
    # Gestione automatizzata dei generatori statici (sgen1-7)
    for i in range(1, 8):
        sgen_name = f"sgen{i}"
